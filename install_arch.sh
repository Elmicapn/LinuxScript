...
# Chiffrement LUKS
echo -n "$LUKS_PASSWORD" | cryptsetup luksFormat --type luks1 "$LUKS_PART"
...
# Monter les fichiers systèmes pour le chroot
mount --types proc /proc "$MOUNT_POINT/proc"
mount --rbind /sys "$MOUNT_POINT/sys"
mount --rbind /dev "$MOUNT_POINT/dev"
mount --rbind /run "$MOUNT_POINT/run"

# Installation du système de base avec une mise à jour avant
pacman -Sy reflector --noconfirm
reflector --latest 10 --sort rate --save /etc/pacman.d/mirrorlist
pacstrap "$MOUNT_POINT" base base-devel linux linux-firmware lvm2 cryptsetup vim nano sudo networkmanager grub efibootmgr

# Génération du fstab
genfstab -U "$MOUNT_POINT" >> "$MOUNT_POINT/etc/fstab"

# Chroot dans le système et configuration
...
echo "GRUB_CMDLINE_LINUX=\"cryptdevice=UUID=$(blkid -s UUID -o value $LUKS_PART):$CRYPT_NAME root=/dev/mapper/$VG_NAME-$LV_ROOT\"" > /etc/default/grub
